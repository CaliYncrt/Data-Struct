...
void RentADT::rentComputer(ComputerADT& computerADT, int customerId, const string& computerName) {
...
int RentADT::getNextID() {
    return rents.empty() ? 1 : rents.back().id + 1;
}

void RentADT::loadRents() {
    ifstream file(rentPath);
    if (!file) {
        cerr << "Unable to open file: " << rentPath << endl;
        return;
    }
